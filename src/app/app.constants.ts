...
// Wayland gives clients no way to request "always above other windows" — that
// stacking decision is the compositor's alone (KDE's "Keep Above Others" is a
// KWin-side window rule the user sets themselves). setAlwaysOnTop() is a
// silent no-op there on every Wayland compositor, so the setting is hidden
// instead of offering a toggle that appears to do nothing. Mirrors
// electron/common.const.ts.
